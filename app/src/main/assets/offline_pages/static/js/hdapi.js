...
(function($) {
  ...
  /* 祈福: 'http://cre.test.hd123.cn:8280' */
  ...
  function POST(url, data, callback)
  {
    ...
    $.ajax({
      ...
      if(result.readyState === 0) {
        error_msg = '网络异常，请重试！';
      } else if(result.readyState === 1 || xhr.readyState === 2) {
        error_msg = '网络异常，请求发出前出现异常';
      } else if(result.readyState === 3) {
        error_msg = '网络异常，解析响应内容时异常';
      ...
      if(result.readyState === 4) {
        if(result && result.responseJSON && result.responseJSON.message)
          ...
          callback({success:false,message: result.responseText});
        else
          callback({success:false,message: error_msg});
      } else {
          callback({success:false,message: error_msg});
      }
    });
    ...
    $.ajax({
      ...
      if(result.readyState==4) {
        if(result && result.responseJSON && result.responseJSON.message)
          ...
          callback({success:false,message: result.responseText + '(' + result.status + ')'});
        else
          callback({success:false,message: error_msg});
      } else {
          callback({success:false,message: error_msg});
