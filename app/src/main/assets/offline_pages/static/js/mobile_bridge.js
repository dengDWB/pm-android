...
window.MobileBridgeWithAndroid = {
  ...
  callNativeScanView: function() {
    console.log(window.MobileBridgeWithAndroid.version());

    if(window.AndroidJSBridge && typeof(window.AndroidJSBridge.callNativeScanView) === "function") {
      window.AndroidJSBridge.callNativeScanView('仪表读数', 'offline://meter.new.html', 'meterid', false, true);
    } else {
      alert("Error 未定义接口： addSubjectMenuItems");
    }
  }
...
window.MobileBridgeWithIOS = {
  ...
  callNativeScanView: function() {
    console.log(window.MobileBridgeWithIOS.version());
    MobileBridgeWithIOS.connectWebViewJavascriptBridge(function(bridge) {
      bridge.callHandler('callNativeScanView', {'bannerTitle': '仪表读数', 'redirectUrl': 'offline://meter.new.html', 'paramName': 'meterid', 'useNativeInput': false, 'redirectToSubjectView': true}, function(response) {});
    })
  }
};
...
window.MobileBridge = {
  ...
  callNativeScanView: function() {
    console.log(window.MobileBridge.version());
    window.MobileBridge.pageLink('仪表读数', 'offline://meter.new.html', 0);
  }
};
